...
""" functions for handling mask sheepStat"""

def get_avg_sheepStat(id, mapStat, mask_sheepStat):
    sheep_count = [0, 0, 0, 0]
    left_sheep = [SHEEP_NUM, SHEEP_NUM, SHEEP_NUM, SHEEP_NUM]
    avg_sheepStat = copy.deepcopy(mask_sheepStat)
    for i in range(BOARD_SIZE):
        for j in range(BOARD_SIZE):
            if 0 < mapStat[i][j] <= 4:
                sheep_count[int(mapStat[i][j] - 1)] += 1
    for i in range(BOARD_SIZE):
        for j in range(BOARD_SIZE):
            if 0 < mapStat[i][j] <= 4 and mapStat[i][j] != id:
                index = int(mapStat[i][j] - 1)
                sheepNum = np.floor(SHEEP_NUM / sheep_count[index])
                if left_sheep[index] >= 2 * sheepNum:
                    avg_sheepStat[i][j] = sheepNum
                    left_sheep[index] -= sheepNum
                else:
                    avg_sheepStat[i][j] = left_sheep[index]
    return avg_sheepStat
...
def MTCS(playerID, mapStat, mask_sheepStat):
    sheepStat = get_avg_sheepStat(playerID, mapStat, mask_sheepStat)
    flag = True
    for i in range(BOARD_SIZE):
        for j in range(BOARD_SIZE):
