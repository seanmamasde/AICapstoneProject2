...
def valid_dir(mapStat, sheep):
    dirs = []
    for i in range(9):
        if sheep[0] + ALL_DIR[i][0] >= 0 and sheep[0] + ALL_DIR[i][0] < BOARD_SIZE and sheep[1] + ALL_DIR[i][1] >= 0 and \
                sheep[1] + ALL_DIR[i][1] < BOARD_SIZE:
...
def get_rank(mapStat):
    ...
    for i in range(4):
        areas = find_connected_components(mapStat, i + 1)
        sum_area = sum(area_size ** 1.25 for area_size in areas)
        scores.append(np.round(sum_area))
    ...
    combined_scores = [team_score, opponent_score]
    sorted_indices = sorted(range(len(combined_scores)), key=lambda x: combined_scores[x], reverse=True)
    rank = [0] * len(scores)

    rank[0] = rank[2] = sorted_indices.index(0)
    rank[1] = rank[3] = sorted_indices.index(1)
...
def Simulate(node):
    ...
    rank = get_rank(mapStat)
    # calculate the final score for the team and opponents
    score = [0] * 4
    team_final_score = FINAL_SCORE[rank[0]]
    opponent_final_score = FINAL_SCORE[rank[1]]

    # assign the calculated scores to the appropriate players
    score[0] = score[2] = team_final_score
    score[1] = score[3] = opponent_final_score
